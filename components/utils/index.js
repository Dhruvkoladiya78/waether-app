import { transformDateFormat } from "./date";
...
export const descriptionToIconName = (desc, descriptions_list) => {
  ...
  console.log(iconName,"checkiconname------>>>>")
...
export const getTodayForecastWeather = (response,descriptions_list) => {
  let all_today_forecasts = [];

  const date = new Date();
  const current_date = transformDateFormat();
  let current_datetime = Math.floor(date.getTime() / 1000);
